# Margdarshak
## Real-Time Public Safety & Dynamic Route Optimization Platform

**Comprehensive Project Overview, Technical Documentation & Algorithm Specification**
...
3. [The Margdarshak Solution](#3-the-margdarshak-solution)
4. [Key Features](#4-key-features)
5. [Mathematical Model & Detailed Algorithms](#5-mathematical-model--detailed-algorithms)
...
8. [Comprehensive API Specifications](#8-comprehensive-api-specifications)
9. [Verification & Performance Benchmarks](#9-verification--performance-benchmarks)
...
**Margdarshak** is an intelligent, real-time public safety navigation and spatial intelligence platform. Unlike standard navigation tools (e.g., Google Maps or Apple Maps), which optimize paths strictly on the basis of travel distance and traffic congestion, Margdarshak prioritizes **user physical safety**.

By aggregating government crime records (FIRs), real-time crowdsourced incident reports, environmental factors (lighting and time of day), and topological road network spatial data, Margdarshak dynamically computes multi-layered risk profiles for road stretches and evaluates navigation routes using a customized **Safety Index**.
...
Urban mobility in developing and expanding cities suffers from significant spatial safety asymmetry. Navigational tools frequently direct pedestrians and drivers through high-crime corridors, unlit back alleys, or hazardous zones simply because those routes save a few seconds or meters.

### 2.2 Core Challenges

1. **Blind Route Optimization** — Conventional routing algorithms compute shortest paths using Dijkstra or A* on pure spatial distance and traffic velocity, ignoring crime density, harassment risk, or dark spots.
...
3. **Unverified Crowdsourced Reports** — Pure user-reporting systems are vulnerable to spam, false alarms, and unverified inputs.
4. **Lack of Dynamic Routing Feedback** — Risk scores must actively influence routing time estimates and path choices without paralyzing traffic flow.

---

## 3. The Margdarshak Solution

Margdarshak addresses these challenges through a holistic architecture that combines geospatial data processing, real-time crowdsourced telemetry, and multi-variable statistical risk modeling.
...
* **Reddit-Style Confidence-Weighted Crowdsourcing** — Real-time upvoting and downvoting prevents spam while dynamically adjusting report severity.
...
* **Spatial Buffer Incident-to-Route Mapping** — Uses a 350 m spatial buffer across full GeoJSON polyline coordinates to dynamically compute path-specific hazards.
* **Speed Penalty & Adjusted ETA** — Adjusts maximum allowed speeds based on safety risk, penalizing dangerous routes in time calculations so that users are naturally guided toward safer alternatives.
...
3. **OSRM Polyline Buffer Hazard Matching**
   - Matches live crowdsourced reports against alternate candidate routes using a 350 m spatial buffer across route GeoJSON polylines.

4. **Dynamic Speed Penalty & ETA Slowdown**
   - Calculates safety travel penalties on risky segments, naturally guiding users toward the safest available route.
...
## 5. Mathematical Model & Detailed Algorithms

### 5.1 Multi-Layer Risk Scoring Engine
...
#### 5.1.1 Layer A: Government Historical Record Score ($A$)
...
#### 5.1.2 Layer B: Community Real-Time Telemetry Score ($B$)
...
1. **Computed Report Severity** — Translates category bounds $(\text{Sev}_{\min}, \text{Sev}_{\max})$ and user input rating $R \in [1, 5]$ into a continuous severity rating:

   $$\text{Severity} = \text{Sev}_{\min} + (\text{Sev}_{\max} - \text{Sev}_{\min}) \cdot \frac{R - 1}{4}$$

2. **Reddit-Style Report Confidence Score** — Measures the reliability of user submissions based on community feedback (upvotes $U$, downvotes $D$):
...
#### 5.1.3 Layer C: Environmental Context Score ($C$)
...
### 5.2 Spatial 1-km Topological Road Stretch Traversal Algorithm
...
### 5.3 Route Safety Evaluation & Spatial Buffer Hazard Filtering
...
### 5.4 Dynamic Speed Penalty & Adjusted ETA Calculation
...
* At $\text{Risk Score} = 50$, $\text{Adjusted Speed} = 0.70 \cdot \text{Base Speed}$ (a 30% slowdown penalty).
...
| **Caching Layer** | Redis | Session caching and spatial lookup acceleration |
| **Authentication** | OAuth2 + JWT (RS256/HS256) | Secure user authentication and token handling, backed by Expo SecureStore on the client |
...
#### `POST /auth/login`
* **Description**: Authenticates a user and returns JWT bearer tokens.
...
1. **Safety Route Selection** — Under simulated high-risk hazard placement (severity 4 incidents), the route evaluation engine successfully shifted the recommended path (`recommended_route_index`) from the shortest-distance route (Safety Index of 50.0%) to a safer alternative route (Safety Index of 85.0%).
...
Built with care, for public safety navigation.

---
*Master documentation compiled by merging the project's technical specification and repository README into a single, professional reference document.*
